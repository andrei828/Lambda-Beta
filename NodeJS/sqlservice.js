...
sqlservice.addTodo = function(callback, todo) {
	...
	var add_todo = 'INSERT INTO todos(id_user, content, checked) VALUES ((?), (?), (?))'
	this.db.run(add_todo, todo.id, todo.content, todo.checked)
...
sqlservice.removeNoteById = function(callback, note) {
	var query = 'DELETE FROM notes WHERE id_user=(?) AND name=(?) AND tag=(?)'
	this.db.run(query, note.id, note.title, note.content)

	var query = 'SELECT * FROM notes WHERE id_user=(?)'
	this.db.all(query, callback)
}

sqlservice.removeTodoById = function(callback, todo) {
	var query = 'DELETE FROM todos WHERE id_user=(?) AND content=(?) AND checked=(?)'
	this.db.run(query, todo.id, todo.content, todo.checked)
...
sqlservice.removeEventById = function(callback, event) {
	var query = 'DELETE FROM events WHERE id_user=(?) AND title=(?) AND day=(?) AND month=(?) AND year=(?)'
	this.db.run(query, event.id, event.title, event.day, event.month, event.year)
